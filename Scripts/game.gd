...
func _ready():
	...
	_spawn_character(Vector2i(9, 9), redfox_data)
	_spawn_character(Vector2i(0, 0), orangefox_data)
...
func _on_mouseover(cell: Vector2i):
	...
	if mode == 'moving':
		...
		var active_character_cell = active_character.get_current_cell()
...
func _on_mode_updated(new_mode: String):
	...
	if mode == 'attack':
		board.display_attack_range(turn_manager.active_character.attack_range, turn_manager.active_character.get_current_cell())
	elif mode == 'moving':
		board.clear_highlight()
	elif mode == 'idle':
		board.reset_travel_path()
		board.display_move_range(turn_manager.active_character.movements, turn_manager.active_character.get_current_cell())
	
func _on_turn_started(character: Character):
	mode = 'idle'
	ui.update_character_info(character)
	board.display_move_range(character.movements, character.get_current_cell())
	if character.can_move_to(board.hovered_cell):
		board.display_path_between(character.get_current_cell(), board.hovered_cell, character.movements)
